...
class OpenAIProvider(LLMProvider):
    ...
    def generate_report(self, context: dict, language: str = "en") -> str:
        ...
        language_instruction = "Write the report in Simplified Chinese." if language == "zh" else "Write the report in English."
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": "You create risk-aware intelligence briefs from supplied JSON evidence."},
                {"role": "user", "content": f"{language_instruction}\nEvidence JSON:\n{context}\n\nDraft format to preserve:\n{fallback}"},
            ],
            temperature=0.2,
        )
