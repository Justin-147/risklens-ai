...
def item(item_id: str, source: str = "Synthetic Regulator", source_type: SourceType = SourceType.regulatory, tags=None) -> IntelligenceItem:
    return IntelligenceItem(
        ...
        evidence_level=EvidenceLevel.primary if source_type == SourceType.regulatory else EvidenceLevel.secondary,
    )
...
def test_severity_influences_ranking():
    ...
    low = item("low", source="Synthetic Industry Media", source_type=SourceType.media, tags=["opportunity_signal"])
...
def test_report_monitoring_actions_vary_by_risk_tag():
    items = [
        ...
        enrich_items_with_severity([item("market", source_type=SourceType.market, tags=["market_risk"])])[0],
    ]
    context = build_report_context("Financial Services", items)
    report = MockLLMProvider().generate_report(context)
    actions = [line for line in report.splitlines() if line.startswith("- Suggested monitoring action:")]
...
def test_fetcher_failure_records_partial_success_without_crash(monkeypatch):
    ...
    monkeypatch.setattr("risklens.agent.orchestrator.build_tool_registry", lambda mock=False, simulate_gap=False: registry)
...
def test_simulate_gap_uses_distinct_output_files():
    ...
    gap = run_agent("financial_services", mock=True, max_iterations=3, max_items=8, simulate_gap=True)
